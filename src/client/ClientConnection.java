            ...
            try {
                while (!(command = fromKeyboard.nextLine()).equals("exit")) {
                    ...
                    switch (parsedCommand[0]) {
                        ...
                        case "import":
                            try {
                                toServer.writeUTF(importingFile(parsedCommand[1]));
                                toServer.flush();
                                System.out.println((String) fromServer.readObject());
                        ...
                        case "remove_all":
                            if (parsedCommand.length == 2) {
                                ...
                                toServer.flush();
                                toServer.writeObject(makeObjectRoom(parsedCommand[1]));
                                toServer.flush();
                                System.out.println((String) fromServer.readObject());
                            } else System.out.println("Пропущен аргумент.");
                            break;
                        case "insert":
                            if (parsedCommand.length == 2) {
                                String[] args = parsedCommand[1].trim().split(" ", 2);
                                if (args.length == 2) {
                                    toServer.writeUTF(command);
                                    toServer.flush();
                                    toServer.writeObject(makeObjectRoom(args[1]));
                                    toServer.flush();
                                    System.out.println((String) fromServer.readObject());
                                } else System.out.println("Пропущен один из аргументов.");
                            } else System.out.println("Пропущены аргументы.");
                            break;
                        default:
                            toServer.writeUTF(command);
                            toServer.flush();
